...
function openPanel(): void { panelOpen = true; emit() }
function closePanel(): void { panelOpen = false; emit() }
...
function SidebarButton(props: any): React.ReactElement {
...
function rowView(
  ...
  const sub = [r.origin === 'subagent' ? '🧩 子代理' : '💬 对话', when, tokens].filter(Boolean).join(' · ')
  const isConfirm = confirmId === r.sessionId
  return h('div', { 'data-arch-row': true, key: r.sessionId },
    h('div', { 'data-arch-row-main': true },
      h('div', { 'data-arch-title': true, title: r.sessionId }, title),
      h('div', { 'data-arch-sub': true }, sub),
    ),
    h('div', { 'data-arch-actions': true },
      h('button', { 'data-arch-btn': true, 'data-arch-restore': true, onClick: () => restore(r.sessionId) }, '↩ 恢复'),
      h('button', {
        'data-arch-btn': true,
        'data-arch-danger': isConfirm,
        onClick: () => { if (isConfirm) remove(r.sessionId); else setConfirmId(r.sessionId) },
      }, isConfirm ? '⚠ 确认删除' : '🗑 删除'),
    ),
  )
}

function ArchivePanel(props: { onClose: () => void }): React.ReactElement {
  ...
  const restore = (id: string): void => {
    apiPost(API.restore, { sessionId: id })
      .then(load)
      .catch((e) => setError((e && e.message) ? e.message : String(e)))
  }

  const remove = (id: string): void => {
    apiPost(API.delete, { sessionId: id })
  ...
  return h('div', { 'data-arch-mask': true, onClick: props.onClose },
    h('div', { 'data-arch-panel': true, onClick: (e) => e.stopPropagation() },
      h('div', { 'data-arch-head': true },
        h('strong', null, '🗄️ 会话归档'),
        h('span', { 'data-arch-spacer': true }),
        h('button', { 'data-arch-btn': true, onClick: load, disabled: loading }, loading ? '⟳…' : '⟳ 刷新'),
        h('button', { 'data-arch-btn': true, onClick: props.onClose }, '✕'),
      ),
      error ? h('div', { 'data-arch-error': true }, '操作失败: ' + error) : null,
      h('div', { 'data-arch-list': true },
        loading && !items ? h('div', { 'data-arch-empty': true }, '加载中…')
          : rows.length === 0 ? h('div', { 'data-arch-empty': true }, '没有归档的对话。\n在会话列表右键（或更多菜单）选择「归档会话」，即可把对话暂存到这里。')
          : rows.map((r) => rowView(r, confirmId, setConfirmId, restore, remove)),
      ),
      items ? h('div', { 'data-arch-foot': true },
        '共 ' + String(rows.length) + ' 个归档对话 · 恢复后回到原位置 · 删除后不可恢复',
      ) : null,
    ),
  )
}
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () =>
    ctx.slots.register({ name: 'sidebar.footer.action', id: 'session-archive', order: 6, label: () => '归档' },
      (props: any) => h(SidebarButton, props),
    ),
  ), 'dsh-session-archive: entry')

  ctx.effect(() => ctx.slots.inject('shell.overlay', () =>
    ctx.slots.register({ name: 'shell.overlay', id: 'session-archive', order: 7 },
      () => {
        const [open, setOpen] = React.useState(panelOpen)
        React.useEffect(() => subscribe(() => setOpen(panelOpen)), [])
        if (!open) return null
