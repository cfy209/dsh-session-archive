...
async function readJsonBody(req: any, maxBytes = 256 * 1024): Promise<any | undefined> {
  const chunks: Buffer[] = []
  let size = 0
...
function requireLoopback(req: any, res: any): boolean {
...
function requireMethod(req: any, res: any, method: string): boolean {
...
export function apply(ctx: AppContext): void {
  ...
  async function listArchived(): Promise<any[]> {
    const sessionQuery = ctx.get('sessionQuery')
    const wr = ctx.get('workspaceRegistry')
    const cache = ctx.get('sessionProjectionCache')
    if (!sessionQuery || !wr) return []

    const archivedIds: string[] = Array.isArray(wr.archivedSessionIds) ? wr.archivedSessionIds : []
    if (archivedIds.length === 0) return []

    const records: any[] = await sessionQuery.listSessions()
    const byId = new Map(records.map((r: any) => [r.header.id, r]))

    const titles = new Map<string, string>()
    try {
      const tRes: any[] = await sessionQuery.readTitleSnapshots(archivedIds)
  ...
  async function removeFromArchived(sessionId: string): Promise<boolean> {
    const wr: any = ctx.get('workspaceRegistry')
    if (!wr || typeof wr.requireState !== 'function' || typeof wr.setState !== 'function') return false
    const state = wr.requireState()
    if (!state || !Array.isArray(state.archivedSessionIds)) return false
    const next = state.archivedSessionIds.filter((id: string) => id !== sessionId)
    if (next.length === state.archivedSessionIds.length) return true // 已不在归档集
  ...
  async function removeFromWorkspaces(sessionId: string): Promise<void> {
    ...
    for (const [wid, rec] of table.entries()) {
      if (rec && Array.isArray(rec.sessionIds) && rec.sessionIds.includes(sessionId)) {
        await table.update(wid, (cur: any) => ({
          ...cur,
  ...
  const routes = [
    {
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          ...
          writeJson(res, 500, { error: String((e as Error)?.message || e) })
        }
      },
    },
    {
      ...
      handler: async (req: any, res: any) => {
        ...
        if (!body || typeof body.sessionId !== 'string') return writeJson(res, 400, { error: 'missing sessionId' })
        try {
          const wr = ctx.get('workspaceRegistry')
          if (!wr) return writeJson(res, 500, { error: 'workspaceRegistry unavailable' })
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          ...
          if (sessionQuery) {
            try {
              const records: any[] = await sessionQuery.listSessions()
              const rec = records.find((r: any) => r.header.id === body.sessionId)
              live = !!(rec && rec.live)
            } catch { /* 查不到就继续 */ }
          }
          if (live) return writeJson(res, 400, { error: '进行中的会话不能删除，请先结束对话' })
  ...
  ctx.logger?.info?.('[' + name + '] 路由就绪: ' + routes.map((r: any) => r.path).join(', '))
}
